...
1. Stores the system prompt and user's question.
2. Calls the AI model.
3. Checks whether the AI wants to use a tool.
4. If no tool is needed → returns the final answer.
5. If a tool is needed → runs the requested tool.
6. Sends the tool result back to the AI.
7. Repeats.
8. Stops if one of the safety limits is reached."""

import json
import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'Day1', "Training")))
